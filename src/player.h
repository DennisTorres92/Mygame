...
struct Player{
    ...
    void ismove(float32 delta, int shaderid);
    void getstats(int& lvl, int& live, int& dev, int& str, int& mag, std::string& name, int& pclass);
    void spotlight(int shaderid);
    std::string getclass();
    Camera* camera;
    private:
    const float mouseSensitivity = 0.3f;
    bool mvforward = false, mvbackward = false, mvleft = false, mvright = false, mouseright = false, fly = false, bjump = false, spotl = false;
    glm::vec3 position;
    std::string name = "";
